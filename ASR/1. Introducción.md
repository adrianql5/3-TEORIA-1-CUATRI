...
1. **Tareas programadas o periódicas**
   - **at, batch**: Ejecutan comandos en momentos concretos o cuando la carga del sistema es baja.
   - **cron**: Permite ejecutar tareas repetidas en intervalos regulares (minutos, horas, días, semanas, etc.).  
     - Se configura con el comando `crontab` y archivos de configuración como `/etc/crontab`.
     - Los trabajos se pueden definir en directorios como `/etc/cron.daily`, `/etc/cron.weekly`, etc.
     - Para sistemas que no están siempre encendidos, **anacron** ejecuta tareas periódicas pendientes al iniciar el equipo.

2. **Automatización de la configuración de servidores**
   - **rsync**: Sincroniza archivos/directorios, solo copia lo modificado y funciona sobre SSH.
   - **pdsh/pdcp/rpdcp**: Ejecutan comandos y copian archivos en paralelo a varios hosts.
   - **unison**: Sincroniza archivos/directorios entre sistemas Windows y UNIX, en ambos sentidos.
   - **Herramientas de clonación y gestión de imágenes**:
     - **partimage, clonezilla, systemimager**: Permiten clonar discos/particiones, distribuir imágenes y automatizar instalaciones en redes de PCs.
...
- **Programa de copia:**  
  - *Basado en imagen* (ej. `dump`, `dd`): copia sectores del disco, restauración completa pero menos flexible.
